function [parameters, A]=DrawSamples_1(M,N) 
...
for m=1:M
    ...
    parameters(m).distribution = input(['Type of distribution for parameter ',parameters(m).name,' (''normal'',''triangle'', or ''uniform'' are supported): ']);  
    %If they all have different distributions, maybe draw the samples for each parameter at THIS step, vs the loop below..
    ...
    if strcmp(parameters(m).distribution,'normal')==1
        ...
        mmu = input('Enter the mean: '); % TO DO: add check that these are type double - and similarly for other distbn inputs below
        ssigma = input('Enter the variance (square of standard deviation): '); 
        
        x1 = sqrt(2*ssigma)*erfinv(-0.9999)+mmu;

        for n=1:N

            if n~=N
                x2 = sqrt(2*ssigma)*erfinv(2*n*intervalarea-1)+mmu;
            elseif n==N
                x2 = sqrt(2*ssigma)*erfinv(0.9999)+mmu;
            end
