function [prcc,studentT]=VariedPRCC_1(M,N,A,OutputOfInterest,xt,idx)
    ...
    if N>=250
        studentT=zeros(M+1,length(xt)); % initializing matrix to hold the studentT statistic for each parameter at each time
    else
        studentT = [];
